...
//        self.backgroundColor = UIColor(red:0.973,  green:0.588,  blue:0.502, alpha:1)
    ...
    func attachImagesToSpinContentView(imagePaths: [String]) {
        ...
        for (index, image) : (Int, UIImage) in images.enumerate() {
            ...
            // generate view
            let contentView = SpinContentView(title: "yo安安", subtitle: "yo", image: image)
    ...
    convenience init(radius: CGFloat, imagePaths: [String]) {
        self.init(frame: CGRectMake(0, 0, radius * 2, radius * 2))
        self.imagePaths = imagePaths
        attachImagesToSpinContentView(imagePaths)
    }
